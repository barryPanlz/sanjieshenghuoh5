...
<div class="screen">
	...
	<div class="main no_footer_main">
		<div class="hot_time"> 
			<span> <span> <?php if(!empty($activityInfo)){ echo $activityInfo[0]['start_time']?> 至 <?php echo $activityInfo[0]['end_time'];} ?> </span>  </span>
		</div>
		<ul class="cl_center miaosha_ul">
			<?php if(!empty($hotgoods)){ foreach($hotgoods as $k=>$v){?>
	           	<li>
		            <a href="<?php echo \Yii::$app->urlManager->createUrl(['goods/details','goodsId'=>$v['goods_id'],'catId'=>$v['cat_id'],'isPromote'=>$v['is_promote']])?>">
