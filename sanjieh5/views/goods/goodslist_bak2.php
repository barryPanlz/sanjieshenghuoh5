	...
	function ajaxNlist (currentPage,clickde,weburl,keyworlds1) {
		...
		$.ajax({
			...
			success:function(msg){ 
				...
				var allwrap="";
				$.each(obj, function(i) {
					var alianjie="<?php echo \Yii::$app->urlManager->createUrl(['goods/details']);?>?goodsId="+obj[i].goods_id+"&catId="+obj[i].cat_id;
					...
					+"</p><p class='cl_center_rig_p2'><span>"+
						obj[i].goods_price
					+"</span><span>三界石</span></p><p class='cl_center_rig_p2'><em>已兑换</em><em>"+
						obj[i].goods_sales
					+"</em></p></div></a></li>"
					allwrap+=ajaxlist;
				})
