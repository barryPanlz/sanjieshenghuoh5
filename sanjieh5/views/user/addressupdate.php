...
<div class="screen">
	...
	<div class="main no_footer_main">
        <form class="addaddress">
            <input type="hidden" name="address_id" id="address_id" value="<?php echo $updatelist->address_id ?>"/>
			<ul class="form_box">
				<li>
					<div>收货人:</div>
					<input type="text" name="consignee" maxlenght="6" id="consignee" value="<?php echo $updatelist->consignee ?>">
				</li>
				<li>
					<div>手机号码:</div>
					<input type="number" name="mobile" id="mobile" maxlength="11" value="<?php echo $updatelist->mobile ?>">
				</li>
					...
					<div>
                        <select name="province" attr="add" id="province">
                            <?php foreach($prolist as $v){ ?>
                                <option value="<?php echo $v['provinceid']; ?>"
                                    <?php if($v['provinceid'] == $updatelist->province){ echo "selected";}?>
                                >
                                    <?php echo $v['province']; ?>
                                </option>
                            <?php } ?>
                        ...
                        <select name="city" attr="add" id="city">
                            <?php foreach($city_list as $v){ ?>
                                <option value="<?php echo $v['cityid']; ?>" <?php if($v['cityid'] == $updatelist->city){
                                    echo "selected";
                        ...
                        <select name="district" id="district">
                            <?php foreach($area_list as $v){ ?>
                                <option value="<?php echo $v['areaid']; ?>" <?php if($v['areaid'] == $updatelist->district){
                                    echo "selected";
                                }?>><?php echo $v['area']; ?></option>
                            <?php } ?>
                        </select>
                    </div>
					<div><input type="text" placeholder="输入街道、门牌号等详细地址" maxlenght="24" name="address" id="address" value="<?php echo $updatelist->address; ?>"></div>
					<div class="choose">
						<label for="u_checkbox"><input class="choose_default" id="u_checkbox" type="checkbox" <?php if( $updatelist->default == '1'){ echo "checked"; }?>>设为默认地址</label>
					</div>
				</li>
			</ul>
		</form>
