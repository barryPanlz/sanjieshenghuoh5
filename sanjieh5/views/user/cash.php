...
<div class="screen">
	...
	<div class="main no_footer_main">
	   ...
	   <div class="u_list">
		<ul>
			<li>
				...
				<h4>账户余额</h4>
				<span><?php echo empty($oneuser->future_currency)?'0':$oneuser->future_currency; ?></span><span class="b">个</span>
			</li>
			<li>
	  ...
	  <div class="store_tranfer_sum" >
		<form action="<?php echo Yii::$app->urlManager->createUrl(['user/exchange']);?>" method="post" id="payform">
		  	<div class="store_tranfer_box cash_box">
		  		<span> 原始货币</span>
		  		<input type="text" name="original_currency"  value="三界石" readonly="readonly"/>
		  	</div>
		  	<div class="store_tranfer_box cash_box">
		  		<span> 兑换数额</span>
		  		<input type="number" class="store_border" name="exchange_num" id="money" placeholder="输入你要兑换的币种数额" />
		  	</div>
		  	<div class="store_tranfer_box cash_box">
		  		<span> 目标货币</span>
		  		<input type="text" name="target_currency" value="人民币（CNY）" readonly="readonly"/>
		  	</div>	  	
		  	<div class="store_tranfer_box cash_box">
		  		<span> 可兑换</span>
		  		<input type="text" name="" id="revbankaccno" placeholder="输入兑换数额后显示" readonly="readonly" style="border:0"/>
		  		<!-- <input type="text" name="result_num" id="result_num" placeholder="输入兑换数额后显示"/> -->
		  	</div>
		</form>
	  </div>
	  ...
	  <div class="page_book">
            ...
            <?php if($page->currentPage >1){ ?>
                <a href="<?php echo \Yii::$app->urlManager->createUrl(['user/exchange','currentPage' =>$page->currentPage - 1])?>">上一页</a>
            <?php } ?>
            <?php if($page->currentPage < $page->totalPage){ ?>
                <a href="<?php echo \Yii::$app->urlManager->createUrl(['user/exchange','currentPage' =>$page->currentPage +1])?>">下一页</a>
            <?php }?>
		</div>
...
<script>
    $("#btn").click(function(){
    	...
    	$.ajax({
            url:"<?php echo Yii::$app->urlManager->createUrl(['user/exchange']);?>",
	...
	$('#money').blur(function(){
		...
		console.log(money)
		var cont = 0.198;
		var jsuan = parseFloat(money*cont).toFixed(2);
